...
variable "vpc_cidr_block" {
  ...
  type        = string
  default     = "10.0.0.0/18"  # NOTE: for some reason, when terraform creates the network, 4 gets added to the subnet mask, so this is really /22
}
...
variable "vpc_secondary_cidr_block" {
  ...
  type        = string
  default     = "10.52.0.0/12" # NOTE: for some reason, when terraform creates the network, 4 gets added to the subnet mask, so this is really /16
}